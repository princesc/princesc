...
do
    ...
    -- Whitelist HWID (izin hanya device ini)
    local ALLOWED_HWIDS = {
        ["dc213b989f33a61e4268a90989a0ad03d2fcfc1eb0d7150e2074e09bd409be28"] = true,
    }

    local function detect_hwid()
        -- 1) HWID dari executor jika ada
        local ok, hw = pcall(function()
            if typeof(gethwid) == "function" then
                return gethwid()
            end
            if typeof(identifyexecutor) == "function" then
                local name, id = identifyexecutor()
                return tostring(id or name)
            end
            return nil
        end)
        if ok and hw and hw ~= "" then return tostring(hw) end

        -- 2) Fallback: GUID lokal jika pernah disimpan oleh viewer (file API)
        local path = ".prince_device_id"
        if typeof(isfile)=="function" and typeof(readfile)=="function" then
            if isfile(path) then
                local ok2, saved = pcall(readfile, path)
                if ok2 and saved and saved ~= "" then return saved end
            end
        end

        -- 3) Tidak ada cara identifikasi
        return "unknown"
    end

    local myHWID = detect_hwid()
    if not ALLOWED_HWIDS[myHWID] then
        pcall(function()
